...
#define ADXL_X_GAIN 333
#define ADXL_Y_GAIN 326
#define ADXL_Z_GAIN 268
    ...
    if (gpLis2dhDev && sensor_sample_fetch(gpLis2dhDev) >= 0) {
        ...
        if (sensor_channel_get(gpLis2dhDev, SENSOR_CHAN_ACCEL_XYZ, accel) == 0) {
           /*snprintf(acc_buffer, sizeof(acc_buffer),
                     ...
                     decodeVal(&accel[0])/-1000 , decodeVal(&accel[1])/-1000, decodeVal(&accel[2])/-1000);*/
					 snprintf(acc_buffer, sizeof(acc_buffer),
                     ":%.3f:%.3f:%.3f:",
                     decodeVal(&accel[0])/-1000 , decodeVal(&accel[1])/-1000, decodeVal(&accel[2])/-1000);
        }
...
void sensorsInit()
{
    ...
    INIT_SENSOR(adi_adxl345, gpLis2dhDev);
